...
CommandRM ships with default entities and features the user may not need.
Before final validation, derive what should be removed to keep the CRM clean.

**Default CommandRM elements** (candidates for removal):
...
- Existing CommandRM entities marked `"type": "extend"`, not `"type": "create"`.
