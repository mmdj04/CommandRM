...
* commandrm version (package.json)
* React version
* Browser version
...
commandrm is a community project, so pull requests are always welcome, but before working on a large change, it is best to open an issue first to discuss it with the maintainers. In that case, prefix it with "[RFC]" (Request for Comments)
...
You must follow the coding style of the existing files. commandrm uses eslint and [prettier](https://github.com/prettier/prettier). You can reformat all the project files automatically by calling
