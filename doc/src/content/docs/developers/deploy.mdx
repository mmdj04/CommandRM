...
description: This document explains how to deploy CommandRM to production.
---

## Deploying The Backend

The entire backend of CommandRM is hosted on Supabase. The backend is composed of a Postgres database, a REST API, and edge functions. Check out the [Supabase Configuration](../supabase-configuration) section for details.
...
The CRM will be available at `https://<username>.github.io/commandrm/`.
...
CommandRM contains GitHub actions for continuous integration and delivery. To enable these actions, you will
...
> **Note:** The `POSTMARK_*` and `VITE_INBOUND_EMAIL` variables are required for CommandRM's inbound email features. Have a look at the the [inbound email configuration](../inbound-email-configuration) to learn more about their usage and setup.
