...
description: This document explains how to configure inbound email processing in CommandRM.
---
...
CommandRM uses [Postmark](https://postmarkapp.com/) to receive and process inbound emails.
...
CommandRM uses a Supabase Edge Function to handle the webhook and process the received emails. You need to configure your Supabase project to handle the incoming requests.
...
This is necessary because it is used both client-side, to display the Inbound Email in the CommandRM app, and server-side (by the Edge Function) to detect when an email is forwarded to CommandRM.
:::
