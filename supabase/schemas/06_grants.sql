...
-- Function grants
-- Anonymous: Only is_admin() and init_state access (for login flow)
grant all on function public.is_admin() to anon;
grant all on function public.is_admin() to authenticated;
grant all on function public.is_admin() to service_role;

-- Authenticated: All trigger/helper functions
...
-- Table grants
-- Anonymous: NO access to any data tables (RLS would block anyway, but defense in depth)
-- Authenticated: Full access (RLS policies control row-level access)
...
-- View grants
-- Anonymous: Only init_state (for checking if any users exist before login)
...
-- Sequence grants
-- Anonymous: NO access to sequences (not needed)
